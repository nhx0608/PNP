...
> **历史评审记录，不描述当前代码。** 本文针对 2026-09-08 的基线 `adfee9d` 写成；§0–§6 列出的阻断项与方案已随 §7 记录的提交落地，§7 末尾"仍未验证（需 Windows 真机）"中的项此后也已在 Windows 真机上完成（`pnp.cmd`/`gateway.cmd` 真机启动、两引擎真实引擎冒烟各 20/21、Desktop MCP 打开应用冒烟）；真实内网模型与内网 MCP 仍未验证。当前状态以 `engineering/verification/results.json` 为准。
