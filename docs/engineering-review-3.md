...
> **历史评审记录**：针对 `master` 提交 `795b98b` 当时的状态写成，不描述当前代码；§7–§8 的"顶层裁决"是评审稿建议，不等于用户批准。当前状态见 `engineering/verification/results.json`。
...
## 16. 增量审查：`d70ca77`（B 线 Pi RPC 驱动与 Engine Pack，`64c9ae8..d70ca77` 共 9 个提交）

**结论：可以留在 master，但有两处设计级缺口（A、B）在真引擎和内网模型下会直接失分，一处伪成功路径（F）违反"不伪造成功"红线；三者都先按下面的裁决定方案，再由 B 实现。** 范围：5 个 B 提交（`e9c9ef3`、`d419294`、`821e905` 与两次清单刷新）加 4 个 Gitee/GitHub 同步合并；新增 `src/drivers/pi-rpc/{protocol,client,launch,tool-bridge,channel}.ts`，`src/engines/pi/pack.ts` 由占位改为真实实现，`src/runtime/process-host.ts` 允许名单加一个变量名，`scripts/pnp-local.ps1` 加 Pi 分支与 StrictMode 安全取值，`.env.example`、`config/engines/pi.json`、`docs/engines/pi.md`、`verification/coverage.md` 同步；8 个 Pi 适配器测试文件与 `tests/unit/pnp-local.test.ts`（仅 win32）。该头的 CI 六作业全绿（run #62）。安全扫描：新增行无主机名、无凭据、无真实用户名路径（只有 `C:\Users\you\...` 占位）。

**认可（不要动）：** 分层正确——Pack 只做 `openPiSession(input)`，进程一律经 `input.host.start(spec, signal, resources)`，边界检查通过；`prompt` 的 response 只当接受证据，完成以 `agent_settled` 为准；`abort` 的 ACK 不当停止证据；进程意外退出让 run 以 `ENGINE_UNAVAILABLE` 失败而不是伪造完成；`agent_end.stopReason` 位置按真实 0.85.1 纠正（此前恒报 `stop` 的缺陷被真机探针抓出来，正是"声明/探测/验证"分层应有的效果）；未知事件降级为 `unknown`、坏帧隔离；工具集变更用指纹拒绝，与 ACP 的 `ENGINE_BINDINGS_CHANGED` 同思路；`tools.unsupported-transport` 通知与 ACP 同形；`extension_ui_request` 桥接到 `services.interact`；`PI_CODING_AGENT_DIR` 指向会话私有目录（上游文档确认它是整个配置根，不是叠加层）；凭据不进事件与异常文本有测试。启动器的 `Get-OptionalProperty` 与 `[object[]]` 两处是 StrictMode 下的真修复。

**裁决：**

**A. 权限策略没有覆盖 pi 内建工具（P0，直接对应"工具权限允许/拒绝"用例）。** pi 没有内建权限系统（`docs/research/T02-pi-harness.md` 第 242 行；上游 `extensions.md` 同）。现状：生成的扩展只对网关注入的 `cli`/`native` 工具做一个泛化的 `ctx.ui.confirm`，pi 自己的 `bash`/`write`/`edit`/`read` 无条件执行；`IntegrationContext.permissions`（`common.permissions`、`PNP_CONFIGURED_POLICY_OVERRIDES`）在 Pi 路径上从未被读取；桥接的 `operation` 是常量 `pi.extension.confirm`，按操作类的策略无从命中，`deny` 对内建工具不可达。同一份 `write: ask` 在 OpenCode 下会产生一次 `permission` 请求，在 Pi 下会静默执行——评测方切换引擎就会看到两种行为。方案：生成的扩展注册 `tool_call` 钩子（上游文档：返回 `{ block: true, reason?, terminate? }`，钩子抛错即阻断，fail-safe），把 pi 内建工具名映射到策略操作类（`bash`→`shell`，`write`/`edit`→`write`，`read`/`grep`/`find`/`ls`→`read`，其余用工具名），别名表与 OpenCode Pack 共用一份；**判定留在网关**：钩子对每个非 `read` 调用发一次 `ctx.ui.confirm`，`title` 固定为 `pnp:<operation>`，`message` 为 JSON `{tool, operation, patterns}`（`patterns` 取自 `event.input` 的路径/命令字段，对应第 3 节 R7），通道从 `title` 解析出 `operation` 后走现有 `authorize()` + InteractionBroker，于是 `allow` 自动放行（source=policy）、`deny` 自动拒绝、`ask` 才产生客户端 `permission` 请求。`allow` 多一次 stdin/stdout 往返是毫秒级，可接受；若要短路，把有效策略写进 sidecar 让扩展本地放行也行，但 `deny`/`ask` 仍必须回网关。网关注入工具的 `registerTool` 门禁改走同一条钩子路径，只留一套代码。有了钩子就不需要 `--tools` 白名单。

**B. 已解析凭据落盘，违反契约（`docs/spec/contracts.md` 第 159 行："已解析的取值……不落盘、不入日志、不进错误消息"）。** `writePiModelsConfig` 把 bearer 原文写进 `models.json`，`writeToolBridge` 把 `tool.env` 的值写进 `pnp-tools.json`。另有两个连带缺陷：(1) 上游 `models.md`"Value Resolution"（2026-09-08 查阅）规定 `apiKey`/`headers` 的值以 `!` 开头会被当 shell 命令执行、含 `$NAME` 会做环境变量插值，原文凭据既可能被改写，极端情况下会被执行；(2) 只投影了 `authorization` 一个头，而 `ConfiguredProvider.prepare` 会把 `headerEnvironment` 里**所有**头都解析出来（内网模型的 appid 一类头就在这里），Pi 路径把它们静默丢掉，C 的模型在 Pi 下不会通过鉴权。方案：照搬 OpenCode Pack 的 `mapHeadersToEnv`——`models.json` 写 `apiKey: "$PNP_PI_API_KEY"`、`headers: { "<原头名>": "$PNP_PI_HEADER_n" }`（上游"Custom Headers"一节就是这个用法），真值只进 `LaunchSpec.env`；工具 sidecar 同理只写变量名，生成的扩展在 pi 进程内读 `process.env`。`LaunchSpec.env` 在 `open()` 时固定，所以切换到 open 时不在环境里的 provider 不能靠 `set_model` 完成：把模型凭据一并纳入 open 时指纹，不一致时 409 `ENGINE_BINDINGS_CHANGED`（与 ACP 同码，撤销新造的 `ENGINE_TOOLS_IMMUTABLE`），或重启进程。附带一句：Windows 上 Node 忽略 `mode: 0o600`，目录 ACL 是唯一保护，这是不写真值的又一个理由。

**C. `pnp.cmd --engine pi` 不是零人工。** 启动器在没有 `PNP_PI_EXECUTABLE`/`PNP_PI_ENTRY` 时直接失败，与 FAQ R10–R12"裁判不预装、启动器自装"的部署故事不符——OpenCode 走得通的那条路 Pi 走不通。B 已在真机探过 0.85.1，锁定它：`config/engines/pi.json` 写 `engineVersion: "0.85.1"`，`distribution: { kind: "npm-node-entry", packageNameCandidates: ["@earendil-works/pi-coding-agent"], entry: "dist/bundle/cli.js" }`；启动器按 OpenCode 同样的 `npm install --prefix runtime/bootstrap/engines/pi/0.85.1 --ignore-scripts <pkg>@0.85.1`，把 `PNP_PI_ENTRY` 设为解析出的 `cli.js`、`PNP_PI_NODE` 设为固定的 Node，版本核对用 `node cli.js --version`；windows/opencode 冒烟作业旁边加一步 `pnp.cmd bootstrap --engine pi`（第 15 节同款）。锁版本是团队流程动作，由 B 提、用户点头即可；启动器改动是机械的。

**D. 共享改动 `PSExecutionPolicyPreference`：代码接受，因果陈述不成立。** `LocalProcessHost.helper()` 启动宿主时已经带 `-ExecutionPolicy Bypass`（`process-host.ts` 约第 111 行）；这个开关设置的就是进程作用域策略（PowerShell 内部正是用 `PSExecutionPolicyPreference` 承载它），它压过 LocalMachine/CurrentUser 注册表作用域，只输给组策略（MachinePolicy/UserPolicy）——而组策略生效时环境变量同样无效。所以该变量不承载任何机密、放进允许名单无害，保留；但 `process-host.ts` 注释、`docs/engines/pi.md`"发现并修复的公共基线缺陷"、`coverage.md` 三处都把"此前任何引擎的 Windows 真实进程都会失败"写成事实，而描述的复现（"只用允许名单减去该键的环境启动 `job-host.ps1`"）不是 `helper()` 的真实启动方式。请 B 在那台沙箱上经 `LocalProcessHost.start` 本身复现一次，附真实 stderr；复现不出来就把三处措辞降为"某一沙箱上观察到，机制未确认"。另外提交说明第二条（Fastify `setErrorHandler` 泛型）不在 diff 里，master 的 `app.ts` 也不需要它——只是说明文字不准。

**E. 零人工与真值以外的小项（B 顺手修，不阻塞）：**
1. `tool-bridge.ts` 把 `"node:" + "child_process"` 拆成两段来绕 `check-boundaries.mjs` 的文本扫描。扫描确实是对原文正则，拆字符串确实能绕过——但"绕过检查"的写法本身不该出现在仓库里。把扩展模板挪成非 `.ts` 资源文件（例如 `src/drivers/pi-rpc/tool-bridge.template.mjs`，构建期原样拷贝、运行期读取），扫描自然不涉及，模板也能单独测试。
2. `ENGINE_CONFIGURATION_ERROR` 返回 400：运维配置错误客户端无法修，应与 `MODEL_ENVIRONMENT_MISSING` 一样是 503；`ENGINE_TOOLS_IMMUTABLE` 改用 `ENGINE_BINDINGS_CHANGED`；任何新码都要进 `contracts.md` 的状态码表。
3. `openPiSession` 用 `input.session.id` 做 `ownerToken`；会话 id 是对外公开的，所有权记录的持有凭证应像 ACP 一样 `randomUUID()`。
4. `handshake()` 把 `get_state` 的一切失败都吞掉：pi 进程在启动时就死（入口路径错、参数不受支持）时，`send` 会立即以进程退出被拒，`open()` 却仍返回"可用"通道，错误要到第一次 `prompt` 才以"process is not running"出现、归因错位。区分两种失败：进程已退出→以 `ENGINE_HANDSHAKE_FAILED`（ACP 同码）带 stderr 摘录抛出；仅 `get_state` 不受支持→容忍。顺手用 `get_state` 的返回填 `native.engineVersion`，并把 `DECLARED_PROTOCOL_VERSION` 里"~0.84.x, unverified"改成已探测的 0.85.1。
5. 文档同步：`PNP_PI_*` 只出现在 `.env.example` 与 `docs/engines/pi.md`，`INSTRUCTION.md` 环境变量表与 `config/SETTINGS.md` 没有；`QUICKSTART.md` 的"Pi（实现并准备完成后）"应写明当前前置条件（C 落地后删掉）。
6. 公开仓库卫生：`docs/engines/pi.md` 出现人名（"靖诗/黔总"），改用角色字母；Gitee 同步合并的作者名是一串手机号（`18016053081`），历史不重写，但请在 Gitee 侧账号设置 `git config user.name`。

**F. `agent_end` 之后的 2 秒兜底会制造伪成功（P1）。** 兜底计时器到点即把 run 结算为 `completed`；若 `agent_settled` 只是晚到，它会落到**下一轮** run 上（`dispatch` 只看 `active` 与 `promptSubmitted`），把下一轮以空文本提前结算成"完成"。真实 0.85.1 已验证总会发 `agent_settled`，GatewayCore 的运行期限本就是兜底：删除该计时器；若坚持保留，到点只能以 `state: failed`、`finish: unknown`、`nativeStopReason: "agent_settled-missing"` 结算，绝不能是 `completed`。

**记录：** 本节只写裁决，未改 B 的代码（Pi 是 B 的目录，且 B 经 Gitee 同步提交，顶层直接改会制造合并冲突）；A、B、F 三项按顺序做，C 依赖锁版本决定，D、E 随手。E 项（HIS 标识入公开仓库）仍待用户定。
