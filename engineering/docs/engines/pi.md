...
所有者 B。入口 `code/src/engines/pi/pack.ts`，通道 `rpc`，公共契约 1.1.0。驱动实现在 `code/src/drivers/pi-rpc/`：`protocol.ts`（帧解析/编码）、`client.ts`（请求关联）、`launch.ts`（可信配置→LaunchSpec）、`tool-bridge.ts`（原生扩展生成）、`channel.ts`（`EngineSessionChannel` 实现）。
...
- **工具**：pi 的 RPC 协议里没有"运行期新增自定义工具"的命令，因此只有公共 `ToolBinding` 联合类型中的 `cli`/`native` 命令绑定能在 `open()` 时通过 `-e <生成的扩展文件>` 一次性注入（B05）。当前扩展不是 MCP 客户端，`mcp-stdio` 与 `mcp-http` 都不会被转换或序列化；它们会被收集为不含 URL、命令、请求头或环境变量值的 `{id, transport, reason}`，并在首次 `run()`、发送任何 prompt 之前通过已等待的原生事件 `pi/tools.unsupported-transport` 报告。报告失败会阻止 prompt，不会把缺少的工具静默当成可用。`run()` 对**全部**绑定（包括未投影的 MCP 绑定）计算递归规范化、传输感知的 SHA-256 指纹；指纹覆盖 id/transport/sideEffect/timeoutMs/inputSchema，以及命令、参数顺序、环境变量键值，或 HTTP URL、请求头键值。**用于工具指纹比较的会话状态只保留 SHA-256 摘要**，不保留用于计算的凭据材料；任何工具、超时、传输或凭据轮换都会在模型切换或 prompt 之前以 `ENGINE_TOOLS_IMMUTABLE` 拒绝。
...
`code/src/drivers/pi-rpc/tool-bridge.ts` 只为 `cli`/`native` 命令绑定生成 pi 扩展注册项（`registerTool`）；它没有实现 MCP 客户端，因此不得把 `mcp-stdio` 误当普通命令，也不得宣称支持 `mcp-http`。受支持的命令执行时用 `child_process.execFile`（**在 pi 自身进程内**，不是本适配器进程发起）以显式 argv 数组调用 `command/args`，不拼接 shell 字符串。`sideEffect !== "read"` 的工具在有 UI 桥时先 `ctx.ui.confirm(...)`（映射为 RPC `extension_ui_request`，`channel.ts#bridgeInteraction` 转发给公共 `services.interact()` 走组织授权，再用 `extension_ui_response` 回复 pi）；没有 UI 桥（`-p`/`json` 模式，RPC 模式下通常有 UI 桥）时直接拒绝执行，不静默放行。命令绑定的环境变量值不写进生成的扩展源码文本，而是单独落一个 0600 的 `pnp-tools.json` sidecar，扩展在 pi 进程内读取。**这是尚未解决的凭据落盘限制**：sidecar 含明文命令凭据，虽然权限设为 0600 且正常删除 Session 时会清理所属目录，但异常退出后的残留窗口仍需后续设计解决；当前实现不能据此宣称凭据从不持久化。未支持的 MCP 绑定不会进入 sidecar 或扩展源码。
