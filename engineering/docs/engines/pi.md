...
- **`capabilityEvidence: "probed"`**（`code/config/engines/pi.json`，`engineVersion: "0.85.1"`）：本驱动依赖的每一个 pi 侧机制都已对真实安装的 `@earendil-works/pi-coding-agent` 0.85.1 手工核验过（命令与实测输出见下方 B08）：`--mode rpc` 帧格式、`agent_end` 的 stopReason 位置、`models.json` + `PI_CODING_AGENT_DIR` 是唯一可用的端点/凭据注入通道、`apiKey`/`headers` 的 `$VAR` 解析、`-e` 加载 `.ts` 与编译后的 `.js` 扩展、扩展在 pi 进程内用 MCP SDK 连接 stdio MCP 服务器并 `registerTool`、`tool_call` 钩子把内建 `bash` 调用变成 `extension_ui_request{title:"pnp:shell"}` 并按否定答复阻断；以及整条 `PiPack` + 真实 `LocalProcessHost` + 真实 pi 的 open→run→terminate 端到端跑通（Linux）。**"probed" 不是 "verified"**：没有一条连的是真实内网模型端点。B08 的核验是手工命令行复现；此后 `npm run e2e -- --engine pi` 已把对真实二进制的端到端检查固化为可重复的本地命令（不在提交的 CI 工作流里），并在 Windows 原生上通过（下一条）。
...
- **Windows 原生真机证据（2026-09-09，Windows x64 / Node 24.19.0，交付 `settings.json` 原样）**：真实 pi 0.85.1 经网关的 Windows 进程宿主启动，`npm run e2e -- --engine pi --expect-desktop-mcp` 21 项中 20 通过、0 失败、1 跳过（`concurrency/same-session-busy`，由 mock 对照组覆盖）。检查项与 OpenCode 腿相同：就绪、事件流、缺 `directory` 400、无 `model` 的 204、文本轨迹、写文件（审批 `once`）、审批 `reject`、Office MCP `csv_read` 中文与空格路径往返、缺失文件真实错误回到模型、Desktop MCP 工具发现、abort（204 + `cancelled`）、反问/授权列表、会话删除、第二会话、跨会话队列、事件序列。同一构建对本机配置的公网 OpenAI 兼容模型 `live-check` 8/8（`docs/team/handoff-settings-mcp-2026-09-09.md`；修复纯文本 `content` 数组兼容问题之后，修复前 6/8 的失败证据也保留）。`gateway.cmd --engine pi` 与 `gateway.ps1` 在 PATH 上无 Node 时也能启动并就绪。
- **仍未验证（不得据此宣称完成）**：真实内网模型端到端调用（`verified` 唯一还欠的东西）；F12 级别对真实 `pi` 进程树的 Job Object 专项清理记录（冒烟里的 abort 只证明取消后会话回到 idle、下一轮可用，没有单独出具进程树核验记录）；Windows 上 `defaultTools` 里 `powershell` 工具的真实执行（冒烟剧本只调用 `write` 与 MCP 工具，`buildPiSettings` 的 win32 分支只有单元测试证据）；`mcp-http` 传输对真实远端 MCP 服务器（冒烟与自动化测试只覆盖 stdio）；`set_model`/`get_available_models` 在真实版本下的返回结构（本驱动已不再发送 `set_model`，见 B04）。
...
工作包提交时未执行（诚实标记）：Windows 上的任何一项（当时全部在 Linux 上跑，`buildPiSettings` 的 win32 分支只有单元测试证据，`engine-contract` 的真实 PowerShell Job Object 路径当时没有被执行）、真实内网模型调用、真实远端 `mcp-http` 服务器、对真实 `pi` 二进制的自动化测试、`npm run release:check` 与清单刷新。此后已补上：Windows 真机上的单元/适配器测试、真实 pi 冒烟 20/21 与公网模型 live-check 8/8（见文首"实现状态"）。仍未执行：真实内网模型、真实远端 `mcp-http`、`powershell` 工具的真实执行。
