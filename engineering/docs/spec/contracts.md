...
| 409 | `SESSION_BUSY`、`SESSION_UNAVAILABLE`、`GATEWAY_BUSY`、`ENGINE_SESSION_MISMATCH`、`CHANNEL_MISMATCH`、`IDEMPOTENCY_CONFLICT`、`RUN_ALREADY_EXISTS`、`INTERACTION_RESOLVED`、`LATE_EVENT`、`ASSET_DIGEST_MISMATCH`、`EXECUTION_CANCELLED`、`ENGINE_BINDINGS_CHANGED`、`ENGINE_MODEL_SWITCH_UNSUPPORTED` | 状态冲突；`EXECUTION_CANCELLED` 的返回码见 3.3；`ENGINE_BINDINGS_CHANGED` 表示本轮的工具/资产/模型绑定与打开该 Session 时不一致，而该引擎把绑定固定在进程启动时（不静默沿用旧绑定，改为要求新建 Session） |
...
| 502 | `BAD_GATEWAY`、`ENGINE_PROTOCOL_ERROR`、`ENGINE_HANDSHAKE_FAILED`、`ENGINE_MODEL_PROTOCOL_UNSUPPORTED`、`ENGINE_MODEL_ENDPOINT_MISSING`、`HOST_EXITED`、`HOST_BACKPRESSURE`、`EVENT_TOO_LARGE`、`OUTPUT_TOO_LARGE`、`UNMATCHED_TOOL_UPDATE`、`UNMATCHED_TOOL_RESULT`、`DUPLICATE_TOOL` | 引擎协议或调用失败；`ENGINE_HANDSHAKE_FAILED` 只用于打开通道时握手失败（含引擎进程在握手期间退出），不得用它掩盖后续运行期错误 |
