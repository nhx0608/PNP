...
test("a tool call left open by the interrupted engine is not fabricated as failed", async () => {
  const outcome = await cancelInFlight({
  ...
  assert.equal(outcome.services.ofType("tool.finished").length, 0);
  const observed = outcome.services.ofType("tool.observed");
  assert.equal(observed.length, 1);
  assert.equal(observed[0]?.status, undefined);
