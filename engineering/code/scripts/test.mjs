...
const files = walk(join("tests", group)).filter((p) => p.endsWith(".test.ts"));
if (!files.length) throw new Error(`No executable tests in ${group}; empty coverage is not a pass.`);
const result = spawnSync(process.execPath, ["--experimental-strip-types", "--test", ...files], { stdio: "inherit", shell: false });
