...
function duration(name: string, fallback: number, minimum: number, maximum: number): number {
  ...
  if (!Number.isInteger(value) || value < minimum || value > maximum) throw new PnpError("VALIDATION_ERROR", `Invalid ${name}.`, 400);
  return value;
}
 ...
 * immediately instead of blocking the run until the interaction deadline expires.
 */
function questionPolicy(): "auto" | "ask" {
  const raw = process.env.PNP_QUESTION_POLICY;
...
// The settings file names its endpoint and credentials by environment variable. A variable it names but the
...
const shutdown = async () => {
  ...
  try { if (app !== undefined) await app.close(); }
    ...
    catch (error) { process.exitCode = 1; console.error(JSON.stringify({ event: "storage.close.failed", code: asPnpError(error).code })); }
