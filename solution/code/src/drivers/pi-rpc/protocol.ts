 ...
 * `packages/coding-agent/src/modes/rpc/rpc-types.ts` against pi 0.84.x), corrected where a real
 * installed `@earendil-works/pi-coding-agent` 0.85.1 process (`docs/engines/pi.md` records the
 * exact manual reproduction) disagreed with that research doc. Concretely: `message_start`,
 * `message_end`, and `turn_end` carry a nested `message` object rather than a flat `role`, and
 * `agent_end` carries the full `messages` array (no top-level `stopReason`) — the earlier
 * flat-`stopReason` shape never actually occurs on the wire and made every real run report
 * `finish: "stop"` regardless of outcome. Shapes not yet exercised against the real binary
 * remain `declared` evidence until probed the same way. Unknown/forward event types decode
...
/** Minimal fields this driver actually reads off a real pi `message` object. The wire shape
 * carries far more (content parts, usage, timestamps, responseId, ...); only `role`/`stopReason`
 * feed this driver's finish-reason mapping, so only those are decoded (verified against real
 * pi 0.85.1; see `docs/engines/pi.md`). */
export interface PiMessageSummary {
...
export type PiEvent =
  ...
  | { readonly type: "agent_end"; readonly willRetry: boolean; readonly messages: readonly PiMessageSummary[] }
  | { readonly type: "agent_settled" }
  | { readonly type: "turn_start" }
  | { readonly type: "turn_end"; readonly message?: PiMessageSummary }
  | { readonly type: "message_start"; readonly message?: PiMessageSummary }
  | { readonly type: "message_update"; readonly assistantMessageEvent?: AssistantMessageEvent; readonly usage?: Json }
  | { readonly type: "message_end"; readonly message?: PiMessageSummary }
...
/** Decodes the subset of a real pi `message` object this driver reads. Returns `undefined`
 * (rather than throwing) for a missing/malformed `message`, since `message_start`/`message_end`/
 * `turn_end` are only ever consumed as best-effort native pass-through events except where noted. */
function asMessageSummary(value: unknown): PiMessageSummary | undefined {
  if (value === null || typeof value !== "object" || Array.isArray(value)) return undefined;
  const record = value as Record<string, unknown>;
  const role = asString(record.role);
  if (role === undefined) return undefined;
  return { role, ...(record.stopReason === undefined ? {} : { stopReason: asString(record.stopReason) }) };
}
...
export function parsePiFrame(line: string): PiEvent {
  ...
  switch (type) {
    ...
    case "agent_end": {
      ...
      const messages = rawMessages.map(asMessageSummary).filter((message): message is PiMessageSummary => message !== undefined);
      return { type, willRetry: asBoolean(record.willRetry), messages };
    }
    ...
    case "agent_start": case "agent_settled": case "turn_start":
    case "auto_retry_start": case "auto_retry_end":
      return { type } as PiEvent;
    case "turn_end": case "message_start": case "message_end": {
      const message = asMessageSummary(record.message);
      return { type, ...(message === undefined ? {} : { message }) } as PiEvent;
    }
