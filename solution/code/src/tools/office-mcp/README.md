...
- **下一步提示（读完之后该调哪个写工具）**：`docx_extract`/`xlsx_read`/`pptx_extract`/`csv_read`/
  `data_aggregate` 的 summary 行末尾追加一行 `下一步 / next: …`（`guidance.ts`），指名写回同类文档的工具
  与 `outputPath`（另存为新文件的参数）；`.md`/`.txt` 结论明确说明用引擎自带的写文件工具即可。
  只加在 summary 行，不进入 `structuredContent`——它是关于工具目录的常量，不是这次读取的计算结果。
  依据是评测用例 office_011：模型用 `docx_extract` 正确读到 9 段，却从未调用过 `docx_replace_paragraphs`，
  改用引擎自带的 `edit`/`write`，最后写出一个扩展名叫 `.docx` 的纯文本并宣称完成。工具描述里已经写了
  索引"可直接用于 docx_replace_paragraphs"，但描述只在列工具时读一次，下一步是看着结果决定的。
  同一原因下，`doc_verify` 的 `format` 失败在 `actual` 里补上"该用哪个工具重新生成"。
