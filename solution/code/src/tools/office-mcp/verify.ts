...
import { writerRemedyFor } from "./guidance.ts";
...
export async function docVerify(file: string, expectations: VerifyExpectations): Promise<DocVerifyResult> {
  ...
  if (!formatValid) {
    ...
    failures.push({
      ...
      // Naming the writer here, not only in the tool list: this failure is what office_011's run
      // actually produced — plain text under a .docx name, written with the engine's native `write`
      // because the model never found docx_replace_paragraphs. A verdict that says only "invalid"
      // leaves it to guess again, so the remedy travels with the verdict.
      actual: OFFICE_EXTENSIONS.has(extension)
        ? `不是有效的 Office 文档 / not a valid Office document: ${problem}（检测到 / detected: ${detection.kind}）`
          + writerRemedyFor(extension)
  ...
  if (readProblem !== null) {
    failures.push({
      ...
      // A package that opens but will not read is unusable for the same reason and has the same fix.
      actual: `不是有效的 Office 文档 / not a valid Office document: ${readProblem}${writerRemedyFor(extension)}`,
    });
  }
