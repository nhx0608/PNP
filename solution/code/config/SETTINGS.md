...
only its name and whether it is set.

Two rules keep a credential out of the file in the first place, and out of the answers:

- **The loader** requires every variable-name slot — `env`, `headerEnvironment` and every
  `*Environment` field — to hold something shaped like a variable name (`^[A-Za-z_][A-Za-z0-9_]*# PNP settings
...
## Configuration API
  ...
  A pasted secret is refused at load with the field's path, rather than being stored and then
  treated as a variable that merely happens to be unset.
- **The HTTP boundary** additionally refuses, in both directions with `CONFIG_HTTP_UNSAFE_FIELD`
  (400): a field whose name reads like a credential and whose value is a string (`apiKey`,
  `auth_token`, at any depth, `native` and `parameters` included); an argv credential written
  either as one element (`--api-key=…`) or as two (`--api-key`, `…`); a URL carrying userinfo;
  and a query parameter whose *name* reads like a credential. An ordinary query string is fine —
  `?api-version=2024-02-01` is how one major provider routes, and refusing it would make a file
  the gateway happily loads unreadable through the API meant to edit it.

This is a field-name heuristic, not a proof. It does not stop an operator hand-writing a secret
into a field nobody would name that way; nothing in the schema expects a credential value, and
`.\pnp.cmd config` or `runtime\local.env` is where values belong. If a stored field does trip
the rule, `GET /config` and `GET /config/raw` refuse and name the exact field path;
`GET /config/environment` still answers, because it returns names and set-flags rather than any
part of the document.

| Route | Answers |
|---|---|
| `GET /config?engine=<id>` | `{ file, running, effective, provenance, capabilities, effect, effects, warnings }` — every effective value labelled `common`, `core`, `environment` (with the variable name and its set flag) or `default`; `warnings` are the validating parser's non-fatal problems for the stored file |
| `GET /config/raw` | the document as stored, ETag = its sha256 |
| `POST /config/validate` | `{ settings, engines? }` → `{ ok, problems[], effective, provenance, capabilities }`, using the same parser that loads the file, and touching nothing on disk |
...
| `PUT /config` | `{ settings, baseSha256, label? }` → validate for every registered engine, back up, then replace atomically; answers `{ sha256, backup, effect, changed, running }` |
...
`running` reports what the process loaded: `{ engine, sha256, loadedAt, inSync }`. `main.ts`
records that digest at startup, which is the only moment it can be known; when a caller builds the
service without it, `sha256` and `inSync` are `null` rather than a guess derived from whatever
the file happens to contain at the first read.
