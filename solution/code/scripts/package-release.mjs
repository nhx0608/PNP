...
// The four standalone documents an assessor reads: what the system is, how to use it, how to
// configure it, and how to install it where there is no public network. Named individually rather
// than copying docs/ wholesale, so nothing new lands in the package by omission.
for (const file of ["ARCHITECTURE.md", "USAGE.md", "CONFIGURATION.md", "INSTALL-INTRANET.md"]) {
  const source = path.join(engineeringRoot, "docs", file);
  if (existsSync(source)) cpSync(source, path.join(solutionDir, "docs", file));
  else console.log(`(skip) docs/${file} does not exist in this checkout.`);
}
