...
## 先看这里：本包是源码交付，需要装一次依赖

解压后**先执行一次安装**，把 Node.js 运行时、依赖包和两个 Agent 引擎准备好：

```powershell
Set-Location <解压目录>\solution\code
.\pnp.cmd bootstrap --engine opencode
.\pnp.cmd bootstrap --engine pi
```

这一步需要能访问 npm（公网或内网镜像），约 5 分钟，**只需做一次**；之后启动和运行都不再需要网络。

- **内网、没有公网、需要配 npm 镜像或代理** → 看 [`docs/INSTALL-INTRANET.md`](docs/INSTALL-INTRANET.md)，
  里面写了 `npm_config_registry`、`PNP_NODE_DOWNLOAD_URL`、以及完全没有 npm 时怎么把依赖拷进来。
- 装好之后本文第 2 步起照做即可。
...
| 文件 | 讲什么 |
|---|---|
| 本文件 | 部署、启动、调接口、判定完成 —— 评测需要的最小路径 |
| [`docs/INSTALL-INTRANET.md`](docs/INSTALL-INTRANET.md) | **内网安装**：npm 镜像、代理、Node 来源、离线拷贝方案 |
| [`docs/USAGE.md`](docs/USAGE.md) | 使用指南，比本文更细的接口示例与排错表 |
| [`docs/CONFIGURATION.md`](docs/CONFIGURATION.md) | **配置参考**：模型、权限、指令、MCP、skill 等所有可配置项 |
| [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) | 架构与详细设计 |
| [`docs/spec/`](docs/spec/)、[`docs/engines/`](docs/engines/) | 契约规格与两个引擎的接入细节 |
...
要求：Windows 10/11 x64，自带的 Windows PowerShell 5.1，不需要管理员权限。

**安装阶段**（上面那一次 `bootstrap`）需要能访问 npm——公网或内网镜像都行。装完之后，启动和执行任务都不再需要网络，也不需要单独安装 Node.js 或 Git。

Python 是可选的：装了 Python 3.9+ 才会启用 PDF 工具；没有也不影响网关和其余工具，网关会如实报告该工具服务不可用。
...
`gateway.cmd` 直接启动网关本身，不做任何准备工作，所以它要求依赖已经装好——也就是本文开头那一次 `bootstrap` 已经执行过、`dist\` 目录存在。没装就先装：`.\pnp.cmd bootstrap --engine opencode`。

它会自己找到 `bootstrap` 装下的 Node 和两个引擎，因此启动时 PATH 上有没有 `node` 都不影响。
